...
class Compute_covmat():
    def __init__(self,rbins,rres,kuse,nv=[0,2,[0,4]],logspace=True,avg_jn=True,load_data=False,path=None):
        self.rbins = rbins
        self.res = rres
        self.ktemp = kuse
        self.k = kuse
    ...
    def save_jn_data(self,file_path="/projects/blazek_group_storage/zepei/ia_forecast/direct_ia_theory/output/avg_jn/"):
    ...
    def set_jn_data1(self):
        print("Compute Bessel function...")
        if self.avg_jn == True:
            for i in self.nv:
                if isinstance(i,list):
                    print(i)
                    self.rp[str(i)],self.j[str(i)] = self.compute_avg_jns(i)
                elif isinstance(i,int):
                    print(i)
                    self.rp[i],self.j[i] = self.compute_avg_jn(i)
                else:
                    print(i)
                    self.rp[i],self.j[i] = 0,0
        else:
            for i in self.nv:
                if isinstance(i,int):
                    ...
                    self.rp[str(i)],self.j[str(i)] = self.compute_jns(i)
                else:
                    self.rp[i],self.j[i] = 0,0
        return 0
    ...
    def load_jn_data(self,file_path = "/projects/blazek_group_storage/zepei/ia_forecast/direct_ia_theory/output/avg_jn/",numbins=20):
        kload = np.load(file_path+"k.npy")
        rbinsload = np.load(file_path+"rbins.npy")
        if np.array_equal(kload, self.k) == True and np.array_equal(rbinsload, self.rbins) == True:
            ...
            self.j[2] = j2
            self.j["[0, 4]"] = j04
        elif np.array_equal(kload, self.k) == False:
            print("replace self.k with loaded k.")
            self.k = kload
            self.rp[0] = np.load(file_path+"rp_nv0.npy")
            self.rp[2] = np.load(file_path+"rp_nv2.npy")
            self.rp["[0, 4]"] = np.load(file_path+"rp_nv04.npy")
            ...
            print("Need get averaged jn data.")
            #self.set_jn_data()
        return True
