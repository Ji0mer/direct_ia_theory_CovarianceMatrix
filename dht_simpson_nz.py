...
class Compute_covmat():
    ...
    def compute_wgpwgp(self,zuse,W1,W2,W1_bin,W2_bin,nbins=20):
        ...
        for i in range( nbins ):
            for j in range( nbins ):
                if i == j:
                    power_componments = np.trapz( W1_bin_2d*W2_bin_2d*( (self.pgg+self.nzd_bin_2d)*(self.pii+self.nzs_bin_2d)+self.pgi**2 ),zuse,axis=0 )/( W1_int*W2_int )
                    cov[i,j] = simpson( self.k/(2*np.pi)*self.j[2][i]*self.j[2][j]*power_componments,x=self.k )
                else:
                    power_componments = np.trapz( W1_bin_2d*W2_bin_2d*( self.pgg*self.nzs_bin_2d+self.pii*self.nzd_bin_2d+2*self.pgi**2 ),zuse,axis=0 )/( W1_int*W2_int )
                    cov[i,j] = simpson( self.k/(2*np.pi)*self.j[2][i]*self.j[2][j]*power_componments,x=self.k )
        return cov
    ...
    def compute_wgpwpp(self,zuse,W1,W2,W1_bin,W2_bin,nbins=20):
        ...
        for i in range( nbins ):
            for j in range( nbins ):
                power_componments = np.trapz( W1_bin_2d*W2_bin_2d*( 2*self.pgi*(self.pii+self.nzs_bin_2d) ),zuse,axis=0 )/( W1_int*W2_int )
    ...
    def compute_wgpwgg(self,zuse,W1,W2,W1_bin,W2_bin,nbins=20):
        ...
        for i in range( nbins ):
            for j in range( nbins ):
                power_componments = np.trapz( W1_bin_2d*W2_bin_2d*( 2*self.pgi*(self.pgg+self.nzd_bin_2d) ),zuse,axis=0 )/( W1_int*W2_int )
    ...
    def compute_wppwgp(self,zuse,W1,W2,W1_bin,W2_bin,nbins=20):
        ...
        for i in range( nbins ):
            for j in range( nbins ):
                power_componments = np.trapz( W1_bin_2d*W2_bin_2d*( 2*(self.pii+self.nzs_bin_2d)*self.pgi ),zuse,axis=0 )/( W1_int*W2_int )
    ...
    def compute_wppwpp(self,zuse,W1,W2,W1_bin,W2_bin,nbins=20):
        ...
        for i in range( nbins ):
            for j in range( nbins ):
                if i == j:
                    power_componments = np.trapz( W1_bin_2d*W2_bin_2d*( 2*(self.pii+self.nzs_bin_2d)*(self.pii+self.nzs_bin_2d) ),zuse,axis=0 )/( W1_int*W2_int )
                    cov[i,j] = simpson( self.k/(2*np.pi)*self.j["[0, 4]"][i]*self.j["[0, 4]"][j]*power_componments,x=self.k )
                else:
                    power_componments = np.trapz( W1_bin_2d*W2_bin_2d*( 2*(self.pii**2+2*self.pii*self.nzs_bin_2d) ),zuse,axis=0 )/( W1_int*W2_int )
                    cov[i,j] = simpson( self.k/(2*np.pi)*self.j["[0, 4]"][i]*self.j["[0, 4]"][j]*power_componments,x=self.k )
        return cov
    ...
    def compute_wppwgg(self,zuse,W1,W2,W1_bin,W2_bin,nbins=20):
        ...
        for i in range( nbins ):
            for j in range( nbins ):
                power_componments = np.trapz( W1_bin_2d*W2_bin_2d*( 2*self.pgi**2 ),zuse,axis=0 )/( W1_int*W2_int )
    ...
    def compute_wggwgp(self,zuse,W1,W2,W1_bin,W2_bin,nbins=20):
        ...
        for i in range( nbins ):
            for j in range( nbins ):
                power_componments = np.trapz( W1_bin_2d*W2_bin_2d*( 2*(self.pgg+self.nzd_bin_2d)*self.pgi ),zuse,axis=0 )/( W1_int*W2_int )
    ...
    def compute_wggwgg(self,zuse,W1,W2,W1_bin,W2_bin,nbins=20):
        ...
        for i in range( nbins ):
            for j in range( nbins ):
                if i == j:
                    power_componments = np.trapz( W1_bin_2d*W2_bin_2d*( 2*(self.pgg+self.nzd_bin_2d)*(self.pgg+self.nzd_bin_2d) ),zuse,axis=0 )/( W1_int*W2_int )
                    cov[i,j] = simpson( self.k/(2*np.pi)*self.j[0][i]*self.j[0][j]*power_componments,x=self.k )
                else:
                    power_componments = np.trapz( W1_bin_2d*W2_bin_2d*( 2*(self.pgg**2+2*self.pgg*self.nzd_bin_2d) ),zuse,axis=0 )/( W1_int*W2_int )
                    cov[i,j] = simpson( self.k/(2*np.pi)*self.j[0][i]*self.j[0][j]*power_componments,x=self.k )
        return cov