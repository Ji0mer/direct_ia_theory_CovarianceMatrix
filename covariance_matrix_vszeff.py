...
def setup(options):
    ...
    Ng = 1/nbar_dens
    return zeff,zmin,zmax,area_shape,area_dens,sample,rbins,nk,Ng,Np


def execute(block, config):

    zeff, zmin, zmax, area_shape, area_dens, sample, rbins, nk, Ng, Np = config
    ...
    # 转成球面立体角 Ω（单位 sr）
    Omega = (area_shape * u.deg**2).to(u.sr)
    # 全天区到 z 的共动体积（Astropy 会返回 Mpc^3）
    V_allsky_zmax = cosmo.comoving_volume(zmax)      # [Mpc^3]
    V_allsky_zmin = cosmo.comoving_volume(zmin)      # [Mpc^3]
    # 你天区覆盖的壳层体积
    V_shell = (V_allsky_zmax - V_allsky_zmin) * (Omega / (4*np.pi * u.sr))  # [Mpc^3]
    # 如果只要数值（去掉单位）
    V_shell_shape = V_shell.value * h0**3
    
    Omega1 = (area_dens * u.deg**2).to(u.sr)
    # 全天区到 z 的共动体积（Astropy 会返回 Mpc^3）
    V_allsky_zmax = cosmo.comoving_volume(zmax)      # [Mpc^3]
    V_allsky_zmin = cosmo.comoving_volume(zmin)      # [Mpc^3]
    # 你天区覆盖的壳层体积
    V_shell = (V_allsky_zmax - V_allsky_zmin) * (Omega1 / (4*np.pi * u.sr))  # [Mpc^3]
    # 如果只要数值（去掉单位）
    V_shell_density = V_shell.value * h0**3
    
    print("\n\n")
    print(V_shell_density)
    print("\n\n")
